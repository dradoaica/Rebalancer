...
/// <summary>A wrapper class for all start, stop and abort actions. This is only of note for provider library implementers.</summary>
public sealed class OnChangeActions
{
    ...
    public void AddOnStartAction(Action<IList<string>> action) => OnStartActions.Add(action);

    public void AddOnStopAction(Action action) => OnStopActions.Add(action);

    public void AddOnAbortAction(Action<string, Exception> action) => OnAbortActions.Add(action);
}
